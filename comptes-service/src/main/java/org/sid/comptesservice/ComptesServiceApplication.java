...
public class ComptesServiceApplication {
    ...
    CommandLineRunner commandLineRunner(CompteRepository compteRepository, OperationRepository operationRepository) {
        return args -> {
            Compte compte = compteRepository.save(new Compte(null, "4444", Math.random() * 1000 + 10, new Date(), "COURANT", "ACTIVE", null, 1l,null));
            List<Operation> operations = new ArrayList<>();
            operations.add(operationRepository.save(new Operation(null, Math.random() * 10000 + 1000, "vers", compte)));
